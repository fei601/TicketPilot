...
_MANAGE_COMMAND_RE = re.compile(
    ...
    # 评测 mng-4：「查一下这场的配票进度」——配票进度在业务里特指订单履约
    # 状态，但知识库也有《配票是什么意思》规则条目，判别信号是指示词
    # （这场/我的）。裸「配票」不进快路径：规则咨询会被订单域抢走
    r"|(这场|我的).{0,10}?配票"
)

# 疑问句守卫（评测 qa-2）：「可以修改订单里面的观演人吗」命中 修改+订单
# → 硬层 llm=0 直接判 ORDER，疑问句没有任何机会走到 LLM 层。
# 指令+对象句式在陈述句里近零误判；疑问句里对象词是咨询话题不是操作对象
_INTERROGATIVE_RE = re.compile(r"(吗|呢|？|\?)\s*$|能不能|可不可以|是否")
...
用户在查询外部实时信息或进行通用对话。
特征：
- 带具体演出/艺人/场次对象，询问时间/余票/票价（会触发工具调用）
- 打招呼、闲聊、实时生活类请求（天气、附近餐饮）（不会触发工具调用）

### QA（知识域）
用户询问票务规则/术语/流程/费用标准等知识性问题。
特征：
- "实名制是什么意思"、"怎么退票"、"一开二开是什么"、"连坐怎么选"
- 没有具体演出/艺人对象的规则、流程、标准类问题——即使看起来能上网查
  （如"开票时间在哪看"、"退票手续费标准"、"停车场怎么收费"）也归 QA：
  知识库有硬门控，未收录会拒答，不会编造
...
输入：重庆凤凰传奇 10.16 1380连坐 全平台 张三110101********1234
输出：{"domain": "ORDER"}
...
3. 不确定时归类为 AGENT
4. 疑问句问规则/标准/流程（"…吗"、"能不能…"、"…怎么收"）即使含"订单""修改"字样也不是 ORDER，是 QA"""
